...
void main(){
	...
	vec3 normalMapValue = texture(normalMap, exTexCoord).rgb;
	normalMapValue = normalize(normalMapValue * 2.0 - 1.0);

	vec3 lightDir = normalize(lightPos - fragPos);
	float diffuse_component = max(dot(lightDir, normalMapValue),0.0);
	vec3 viewDir = normalize(viewPos - fragPos);
	vec3 reflectDir = reflect(-lightDir, normalMapValue);
	vec3 halfwayDir = normalize(lightDir + viewDir);
	float specular_component = pow(max(dot(normalMapValue, halfwayDir),0.0),shininess_factor * mat.shininess);
